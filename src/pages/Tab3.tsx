...
const Tab3: React.FC = () => {
  ...
  const [items, setItems] = useState({
    ...
    yearpostulacion:getYearPostulacion(),
    direccion: '',
    comuna: '',
    barrio: '',
    ruralurbano: '',
  ...
  const fetchUsers = async (database = db) => {
    if (db) {
      const res = await database.exec(`SELECT * FROM inclusion_ubicacion WHERE id_usuario=${params.ficha}`);
  ...
  useEffect(() => {
    if (people.length > 0) {
      const data = people[0] || {}; // Verifica si hay datos en `people`
      console.log('Loaded data:', data); // Verificar datos cargados
      setItems({
        id_usuario: data.id_usuario || params.ficha,
        yearpostulacion:data.id_usuario || getYearPostulacion(), 
        ...
        tabla: data.tabla || 'inclusion_ubicacion', 
        ...
        tenenciadelavivienda: data.tenenciadelavivienda || ''  // Propiedad que faltaba
      });
    }
  }, [people]);
...
//   };
 ...
 useEffect(() => {
  ...
  const enviar = async (database = db,event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    console.log(items)
    try {
      await db.exec(`INSERT OR REPLACE INTO inclusion_ubicacion 
        (id_usuario, yearpostulacion, direccion, comuna, barrio, ruralurbano, sector, estrato, fecharegistro, usuario, estado, tabla, dirCampo1, dirCampo2, dirCampo3, dirCampo4, dirCampo5, dirCampo6, dirCampo7, dirCampo8, dirCampo9, longitud, latitud, tenenciadelavivienda) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?);`, [
        items.id_usuario, items.yearpostulacion, items.direccion, items.comuna, items.barrio, items.ruralurbano, items.sector, items.estrato, 
        items.fecharegistro, items.usuario, items.estado, items.tabla, items.dirCampo1, items.dirCampo2, items.dirCampo3, 
        items.dirCampo4, items.dirCampo5, items.dirCampo6, items.dirCampo7, items.dirCampo8, items.dirCampo9, 
        items.longitud, items.latitud, items.tenenciadelavivienda
      ]);
  ...
  return (
    ...
    <IonHeader>
      <IonToolbar>
        <IonTitle slot="start">LOCALIZACIÓN</IonTitle>  
        <IonTitle slot="end">USUARIO: <label style={{color:'#17a2b8'}}>{params.ficha}</label> </IonTitle>
      </IonToolbar>
    </IonHeader>
...
<hr />
  ...
  <div className="row g-3 was-validated "> 
              <div className="col-sm">
                    ...
                    <option value=""> SELECCIONE </option><option value="1"> ESTRATO 1 - BAJO-BAJO </option><option value="2"> ESTRATO 2 - BAJO </option><option value="3"> ESTRATO 3 - MEDIO-BAJO </option><option value="4"> ESTRATO 4 - MEDIO </option><option value="5"> ESTRATO 5 - MEDIO-ALTO </option><option value="6"> ESTRATO 6 - ALTO </option><option value="7"> NINGUNO </option><option value="8"> SIN DATO </option>
                    </select>
              </div>
            <div className="col-sm">
              <label  className="form-label" >Tenencia de la vivienda:</label>
              <select className="form-control form-control-sm" onChange={(e) => handleInputChange(e, 'tenenciadelavivienda')} value={items.tenenciadelavivienda} id="tenenciadelavivienda" required>
                <option value="1"> PROPIA </option><option value="2"> ALQUILADA </option><option value="3"> PRESTADA </option><option value="4"> INVADIDA </option><option value="5"> POSESIÓN </option><option value="6"> SUSESIÓN </option><option value="7"> SIN DATO </option><option value="8"> NO APLICA - HABITANTE DE LA CALLE  </option><option value="9"> FAMILIAR </option>                
              </select>            
              </div>
