...
const IngresarIntegrantes: React.FC = () => {
  ...
  const integrantes = queryParams.get('idintegrante');
  const [peopleGrupoFamiliar, setPeopleGrupofamilar] = useState<GrupoFamiliar[]>([]);
  const [db, setDb] = useState<any>(null);
  const [itemsGrupoFamiliar, setItemsGrupoFamiliar] = useState<GrupoFamiliar>({  
    integrantes: '',
    id_usuario: '',
    fecharegistro: '',
    usuario: '',
  ...
  const [items, setItems] = useState<Person>({
    id_usuario:`${Math.random().toString().substr(2, 5)}${localStorage.getItem('cedula')}`,
  ...
  useEffect(() => {
    loadSQL(setDb, fetchUsersGrupoFamiliar);
    loadSQL(setDb,fetchUsers);
  }, []);
console.log(integrantes, 'idintegrante')
  const saveDatabase = () => {
    if (db) {
    ...
    if (db && numeroDocumento) {
      try {
        ...
        if (res[0]?.values && res[0]?.columns) {
          // Si hay resultados, transformar y usar el id_usuario existente
          ...
          if (transformedPeople.length > 0) {
            // Usar el id_usuario existente
            setItems((prevItems) => ({
              ...prevItems,
              ...transformedPeople[0],  // Cargar los datos existentes
              id_usuario: transformedPeople[0].id_usuario,
            }));
          } else {
            // Si no se encuentran datos, mantener el id_usuario actual
            setItems((prevItems) => ({
              ...prevItems,
          ...
          setItems((prevItems) => ({
            ...prevItems,
            numerodedocumento: numeroDocumento,
            id_usuario: '', // Resetea id_usuario si no existe
          }));
        }
        console.log(items)
      } catch (err) {
  ...
  const fetchUsers = async (database = db) => {
    if (db) {
      try {
        const res = await database.exec(`SELECT * FROM inclusion_ciudadano WHERE id_usuario=${integrantes}`);
  ...
  const fetchUsersGrupoFamiliar = async (database = db) => {
    if (db) {
      const res = await database.exec(`SELECT * FROM inclusion_grupofamiliar WHERE id_usuario=${params.ficha} AND integrantes=${integrantes}`);
      
      if (res[0]?.values && res[0]?.columns) {
        const transformedPeople: GrupoFamiliar[] = res[0].values.map((row: any[]) => {
          return res[0].columns.reduce((obj, col, index) => {
            obj[col] = row[index];
            return obj;
          }, {} as GrupoFamiliar);
        });
        setPeopleGrupofamilar(transformedPeople);
      } else {
        setItemsGrupoFamiliar({
          integrantes:items.id_usuario,
          id_usuario: params.ficha,
          fecharegistro: getCurrentDateTime(),
          usuario: localStorage.getItem('cedula'),
          estado: '1',
          tabla: 'inclusion_grupofamiliar',
        });
      }
    }
  };
  ...
  useEffect(() => {
    if (peopleGrupoFamiliar.length > 0) {
      let data = peopleGrupoFamiliar[0] || {};
      setItemsGrupoFamiliar({
  ...
  useEffect(() => {
    console.log("Items updated:", itemsGrupoFamiliar);
  }, [itemsGrupoFamiliar]);
  ...
  const enviar = async (database = db, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      // Verificar si el `id_usuario` ya está definido y si existe en la base de datos
      let idUsuarioActual = items.id_usuario;
  
      if (!idUsuarioActual) {
        // Si `id_usuario` no está definido, intentamos encontrarlo por `numerodedocumento`
        const res = await db.exec(`SELECT id_usuario FROM inclusion_ciudadano WHERE numerodedocumento = ?`, [items.numerodedocumento]);
  
        if (res[0]?.values?.length > 0) {
          // Si existe en la base de datos, usar el id_usuario encontrado
          idUsuarioActual = res[0].values[0][0];
        } else {
          // Si no existe, generar un nuevo id_usuario
          idUsuarioActual = `${Math.random().toString().substr(2, 5)}${localStorage.getItem('cedula')}`;
        }
  
        // Actualiza `items` con el id_usuario determinado
        setItems((prevItems) => ({
          ...prevItems,
      ...
      // Guardar en la tabla `inclusion_ciudadano`
      await db.exec(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?);
      `, [
        idUsuarioActual, items.yearpostulacion, items.tipodedocumento, items.nacionalidad, items.numerodedocumento, items.nombre1, items.nombre2, items.apellido1, items.apellido2,
        ...
        items.telcontactodos, items.fechadenacimiento_verificada, items.fecharegistro, items.tabla, items.usuario, items.estado
      ]);
  
      // Guardar en la tabla `inclusion_grupofamiliar`
      await db.exec(
        `INSERT OR REPLACE INTO inclusion_grupofamiliar 
        (id_usuario, integrantes, fecharegistro, usuario, estado, tabla) 
        VALUES (?,?,?,?,?,?);`,
        [
          itemsGrupoFamiliar.id_usuario,
          idUsuarioActual,
          itemsGrupoFamiliar.fecharegistro,
          itemsGrupoFamiliar.usuario,
          itemsGrupoFamiliar.estado,
          itemsGrupoFamiliar.tabla
        ]
      );
...
<form> 
        {/* <div className="social-card">
          ...
          <span className="value">Idintegrante: {integrantes}</span>
            ...
            <div className="row g-3 was-validated ">
              ...
              <div className="col-sm-6">
                ...
                <select onChange={(e) => handleInputChange(e, 'tipodedocumento')} value={items.tipodedocumento} className="form-control form-control-sm" id="pregunta2_3" aria-describedby="validationServer04Feedback" required>
                  ...
                  <option value="11"> NUIP: NÚMERO ÚNICO DE IDENTIFICACIÓN GrupoFamiliarAL </option>
              ...
              <div className="col-sm-6">
                ...
                <input type="number" onChange={(e) => handleInputChange(e, 'numerodedocumento')} value={items.numerodedocumento} placeholder="" className="form-control form-control-sm  " disabled={integrantes} required />
              </div>
            </div>
          </IonList>
          <IonList>
            ...
            <div className="row g-3 was-validated ">
              <div className="col-sm">
                ...
                <select  onChange={(e) => handleInputChange(e, 'victima')} value={items.victima} className="form-control form-control-sm" id="pregunta2_3" aria-describedby="validationServer04Feedback" required>
                  <option value=""> SELECCIONE </option><option value="8"> ABANDONO O DESPOJO FORZADO </option><option value="10"> AMENAZA A LA VIDA, INTEGRIDAD Y SEGURIDAD GrupoFamiliarAL </option><option value="9"> DAÑO EN BIENES MUEBLES O INMUEBLES (TERRORISMO) </option><option value="7"> DELITOS CONTRA LA INTEGRIDAD Y LIBERTAD SEXUAL </option><option value="1"> DESAPARICIÓN FORZADA </option><option value="4"> DESPLAZAMIENTO FORZADO </option><option value="15"> HOMICIDIO </option><option value="6"> LESIONES GrupoFamiliarALES (PERMANENTES O TRANSITORIAS) </option><option value="11"> MÁS DE UN HECHO VICTIMIZANTE </option><option value="12"> NINGUNA </option><option value="16"> OTRO </option><option value="3"> RECLUTAMIENTO FORZADO Y UTILIZACIÓN </option><option value="2"> SECUESTRO </option><option value="13"> SIN DATO </option><option value="14"> TORTURA O TRATOS CRUELES, INHUMANOS O DEGRADANTES </option><option value="5"> VÍCTIMAS DE MINAS ANTIGrupoFamiliarAL (MAP), MUNICIONES SIN EXPLOSIONAR (MUSE) Y ARTEFACTOS EXPLOSIVOS IMPROVISADOS (AEI) </option>
                </select>
              </div>
            </div>
          </IonList>
